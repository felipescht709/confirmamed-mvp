...
const CriarAgendamentoTool = {
  definition: {
    ...
    function: {
      ...
      description:
        "Agenda uma nova consulta no banco de dados. SÓ DEVE SER CHAMADA após o usuário escolher um horário livre e confirmar os dados.",
      parameters: {
        type: "object",
        properties: {
          paciente_id: {
            type: "integer",
            description:
              "O ID numérico do paciente (recebido no contexto ou na tool upsert_paciente)",
          },
          profissional_id: {
            type: "integer",
            description: "O ID numérico do médico/profissional",
          },
          data_hora_inicio: {
            type: "string",
            description:
              "A data e hora EXATA do início no formato YYYY-MM-DD HH:mm:ss",
          },
          data_hora_fim: {
            type: "string",
            description:
              "A data e hora do término no formato YYYY-MM-DD HH:mm:ss (geralmente 30 mins após o início)",
          },
        },
        required: [
          "paciente_id",
  ...
  async execute({
    ...
    try {
      console.log(
        `[TOOL] Tentando agendar: Paciente ${paciente_id} com Profissional ${profissional_id} as ${data_hora_inicio}`,
      );
      ...
      return await knex.transaction(async (trx) => {
        ...
        const conflito = await trx("consultas")
          ...
          .andWhere(function () {
            this.where("data_hora_inicio", "<", data_hora_fim).andWhere(
              "data_hora_fim",
              ">",
              data_hora_inicio,
            );
        ...
        PushService.notifyUnidade(
          ...
          `A IA agendou o paciente ID ${paciente_id} automaticamente.`,
          { url: "/agenda" },
